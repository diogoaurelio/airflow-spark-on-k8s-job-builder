...
class SparkK8sJobBuilder(object):
    ...
    def add_global_persistent_volume(
        ...
        self._setup_volume_mount(
            executor=True, driver=True, volume_name=volume_name, mount_path=mount_path,
            readonly=readonly
        )
    ...
    def add_configmap_volume(
        ...
        if driver_mount_path:
            self._setup_volume_mount(
                executor=False, driver=True, volume_name=volume_name, mount_path=driver_mount_path,
                readonly=readonly
            )

        if executor_mount_path:
            self._setup_volume_mount(
                executor=True, driver=False, volume_name=volume_name, mount_path=executor_mount_path,
                readonly=readonly
            )
    ...
    def add_executor_empty_dir_volume(
        ...
        self.get_job_params()["volumes"] = existing_volumes
        self._setup_volume_mount(
            executor=True, driver=False, volume_name=volume_name, mount_path=mount_path,
            readonly=readonly
        )
    ...
    def setup_xcom_sidecar_container(self):
        ...
        self._setup_volume_mount(
            executor=False, driver=True, volume_name="xcom", mount_path="/airflow/xcom",
            readonly=False,
        )
    ...
    def _setup_volume_mount(self, executor: bool, driver: bool, volume_name: str, mount_path: str,
                            readonly: bool):
        vm_config = {
            "name": volume_name,
